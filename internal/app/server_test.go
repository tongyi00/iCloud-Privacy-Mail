...
func TestMailboxHTMLPreviewUsesSandboxedIFrame(t *testing.T) {
	...
	for _, want := range []string{
		...
		"frame.srcdoc = mailboxMessageDocument(message.body)",
	} {
		if !strings.Contains(string(page), want) {
			t.Fatalf("mailbox HTML preview is missing %q", want)
		}
	}
